        ...
        <template slot-scope="props">
          <el-form label-position="left" class="demo-table-expand">
            ...
            <el-form-item label="应还日期：">
              ...
              <el-button @click="changePersonReturnDate(props.row)" type="text"
...
import {deleteBorrow, searchBorrow, alertPerson, changeBorrowInfo} from "@/api";
import qs from "qs";
export default {
  name: "AdminBorrows",
  data() {
  ...
  methods: {
    changePersonReturnDate(row){
      console.log(row);
      var borrowDate=row.borrowDate;
      ...
      this.$prompt("请输入规定归还时间", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputValue: row.returnDate,
        inputValidator: value => {if (value=="")return false;return true;},
      })
          .then(({value}) => {
            this.$message({
              type: "success",
              message: "你修改的规定归还时间是: " + value,
            });
            // 修改的信息
            returnDate = value;
            var borrowObj = {borrowDate,bookId , bookName, readerId, readerName, returnDate, realDate};
            changeBorrowInfo(qs.stringify(borrowObj)).then(
                (res) => {
